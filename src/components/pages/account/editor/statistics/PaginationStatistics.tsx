...
const PaginationStatistics = (props: Props) => {
    ...
    const changePage = async (event: ChangeEvent<HTMLSelectElement>) => {
        ...
        navigate(`?name=${props.subject}&page=${event.target.value}&size=${pageSize}`);
    };

    const changePageSize = async (event: ChangeEvent<HTMLSelectElement>) => {
        await queryNewPageSize(event.target.value);
        navigate(`?name=${props.subject}&page=${pageNumber}&size=${event.target.value}`);
    };
...
export default PaginationStatistics;